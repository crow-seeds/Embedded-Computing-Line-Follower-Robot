...
void static SendByte(uint8_t byte){
    ...
    uint8_t byteBackup = byte;
    int i = 0;
    for(i = 0; i < 8; i++){ //goes through all 8 bits of the byte and sends each bit
        if(byteBackup & 0b10000000){
            LCD_SEND_1;
        }else{
            LCD_SEND_0;
        }
        byteBackup = byteBackup << 1;
        LCD_TOGGLE_SCLK; //toggles clock twice
        LCD_TOGGLE_SCLK;
    }
}
